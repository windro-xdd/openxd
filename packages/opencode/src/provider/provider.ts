...
import { CUSTOM_LOADERS, type CustomModelLoader } from "./provider-custom-loaders"
import { loadBaseURL } from "./provider-options"
import { wrapSSE } from "./provider-stream"
import { runPluginAuthLoader } from "./provider-plugin-auth"
...
export namespace Provider {
  ...
  const state = Instance.state(async () => {
    ...
    for (const plugin of await Plugin.list()) {
      ...
      if (auth) {
        const opts = await runPluginAuthLoader(plugin.auth.loader, providerID, database[plugin.auth.provider])
      ...
      if (providerID === "github-copilot") {
        ...
        if (!disabled.has(enterpriseProviderID)) {
          ...
          if (enterpriseAuth) {
            const opts = await runPluginAuthLoader(
              plugin.auth.loader,
    ...
    for (const [providerID, fn] of Object.entries(CUSTOM_LOADERS) as Array<
      [string, (provider: Info) => Promise<any>]
    >) {
      if (disabled.has(providerID)) continue
  ...
  async function getSDK(model: Model) {
    try {
      ...
      options["fetch"] = async (input: any, init?: BunFetchRequestInit) => {
        ...
        const requestTimeout = options["timeout"] === false ? undefined : (options["timeout"] ?? DEFAULT_REQUEST_TIMEOUT)
