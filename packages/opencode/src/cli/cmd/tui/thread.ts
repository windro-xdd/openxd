...
export const TuiThreadCommand = cmd({
  command: "$0 [project]",
  describe: "start openxd tui",
  builder: (yargs) =>
    withNetworkOptions(yargs)
      .positional("project", {
        type: "string",
        describe: "path to start openxd in",
      })
      .option("model", {
        type: "string",
        alias: ["m"],
  ...
  handler: async (args) => {
    ...
    try {
      ...
      // First-run detection — offer boot setup if no identity files exist
      if (await needsBoot()) {
        const readline = await import("readline")
        const rl = readline.createInterface({ input: process.stdin, output: process.stdout })
        UI.empty()
        UI.println(UI.Style.TEXT_INFO_BOLD + "  Welcome to OpenCode!" + UI.Style.TEXT_NORMAL)
        UI.println(UI.Style.TEXT_INFO_BOLD + "  Welcome to OpenXD!" + UI.Style.TEXT_NORMAL)
        UI.empty()
        ...
        if (answer.toLowerCase() !== "n" && answer.toLowerCase() !== "no") {
          // Re-run as `opencode boot`
          const { BootCommand } = await import("@/cli/cmd/boot")
          await BootCommand.handler!({} as any)
          return
        }
        UI.empty()
      }
