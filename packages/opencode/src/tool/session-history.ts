...
- get: Get a specific range of messages by index`,
  ...
  async execute(input, ctx) {
    ...
    const output = (text: string, metadata: Record<string, number> = {}) => ({ title: "", metadata, output: text })
    ...
    if (input.action === "search") {
      if (!input.query) return output("Error: query is required for search action")
      ...
      return output(
        ...
        { totalMessages: allMessages.length, matchCount: matches.length },
      )
    }

    if (input.action === "recent") {
      const start = Math.max(0, allMessages.length - count)
      const recent = allMessages.slice(start)
      const formatted = recent.map((msg, i) => formatMessage(msg, start + i)).filter(Boolean)
