...
async function load(): Promise<Data> {
  ...
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const localDest = path.join(dir, ".openxd", "themes", localThemeFile)
      const globalDest = path.join(Global.Path.config, "themes", globalThemeFile)
      const preloadedDest = path.join(dir, ".openxd", "themes", preloadedThemeFile)
...
`,
  ...
  try {
    ...
    const leaked_global_to_local = await fs
      .stat(path.join(tmp.path, ".openxd", "themes", tmp.extra.globalThemeFile))
      .then(() => true)
      .catch(() => false)
...
`,
  ...
  process.env.OPENXD_PLUGIN_META_FILE = path.join(tmp.path, "plugin-meta.json")
  ...
  try {
    ...
    get.mockRestore()
    wait.mockRestore()
    delete process.env.OPENXD_PLUGIN_META_FILE
  }
})
