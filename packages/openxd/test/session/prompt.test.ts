...
import { NamedError } from "@openxd-ai/util/error"
  ...
  test("does not loop empty assistant turns for a simple reply", async () => {
    ...
    const server = Bun.serve({
      ...
      fetch(req) {
        ...
        calls++
        return new Response(chat("packages/openxd/src/session/processor.ts"), {
          status: 200,
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          const other = await SessionPrompt.prompt({
            sessionID: session.id,
            agent: "build",
            model: { providerID: ProviderID.make("openxd"), modelID: ModelID.make("kimi-k2.5-free") },
            noReply: true,
