...
describe("plugin.install.task", () => {
  test("writes both server and tui config entries", async () => {
    ...
    const server = await read(path.join(tmp.path, ".openxd", "openxd.jsonc"))
    const tui = await read(path.join(tmp.path, ".openxd", "tui.jsonc"))
  ...
  test("does not change configured package version without force", async () => {
    await using tmp = await tmpdir()
    const target = await plugin(tmp.path, ["server"])
    const cfg = path.join(tmp.path, ".openxd", "openxd.json")
  ...
  test("writes to global scope when global flag is set", async () => {
    ...
    expect(await Filesystem.exists(path.join(global, "openxd.jsonc"))).toBe(true)
    expect(await Filesystem.exists(path.join(tmp.path, ".openxd", "openxd.jsonc"))).toBe(false)
  })
  ...
  test("writes local scope under directory when vcs is not git", async () => {
    ...
    expect(ok).toBe(true)
    expect(await Filesystem.exists(path.join(directory, ".openxd", "openxd.jsonc"))).toBe(true)
    expect(await Filesystem.exists(path.join(worktree, ".openxd", "openxd.jsonc"))).toBe(false)
  })
  ...
  test("writes tui local scope under directory when worktree is root slash", async () => {
    ...
    const ok = await run(ctxRoot(directory))
    expect(ok).toBe(true)
    expect(await Filesystem.exists(path.join(directory, ".openxd", "tui.jsonc"))).toBe(true)
  })
  ...
  test("writes only tui config for tui-only plugins", async () => {
    ...
    const ok = await run(ctx(tmp.path))
    expect(ok).toBe(true)
    expect(await Filesystem.exists(path.join(tmp.path, ".openxd", "tui.jsonc"))).toBe(true)
    expect(await Filesystem.exists(path.join(tmp.path, ".openxd", "openxd.jsonc"))).toBe(false)
  })
  ...
  test("force replaces version in both server and tui configs", async () => {
    await using tmp = await tmpdir()
    const target = await plugin(tmp.path, ["server", "tui"])
    const server = path.join(tmp.path, ".openxd", "openxd.json")
    const tui = path.join(tmp.path, ".openxd", "tui.json")
  ...
  test("returns false and keeps config unchanged for invalid JSONC", async () => {
    await using tmp = await tmpdir()
    const target = await plugin(tmp.path, ["server"])
    const cfg = path.join(tmp.path, ".openxd", "openxd.jsonc")
    await fs.mkdir(path.dirname(cfg), { recursive: true })
  ...
  test("returns false when manifest declares no supported targets", async () => {
    ...
    expect(ok).toBe(false)
    expect(await Filesystem.exists(path.join(tmp.path, ".openxd", "openxd.jsonc"))).toBe(false)
    expect(await Filesystem.exists(path.join(tmp.path, ".openxd", "tui.jsonc"))).toBe(false)
  })
