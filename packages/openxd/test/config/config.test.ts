...
const managedConfigDir = process.env.OPENXD_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "openxd.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "openxd.json") {
...
test("ignores legacy tui keys in openxd config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://openxd.ai/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "openxd.json"))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["OPENXD_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { openxd: { options: { apiKey: "{env:OPENXD_CONSOLE_TOKEN}" } } },
        }),
      ),
    ...
    await provideTmpdirInstance(() =>
      Config.Service.use((svc) =>
        Effect.gen(function* () {
          ...
          expect(config.provider?.["openxd"]?.options?.apiKey).toBe("st_test_token")
        }),
      ),
    ).pipe(Effect.scoped, Effect.provide(layer), Effect.runPromise)
  } finally {
    if (originalControlToken !== undefined) {
      process.env["OPENXD_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["OPENXD_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "openxd.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .openxd directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const openxdDir = path.join(dir, ".openxd")
      await fs.mkdir(openxdDir, { recursive: true })
      const agentDir = path.join(openxdDir, "agent")
...
test("loads agents from .openxd/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const openxdDir = path.join(dir, ".openxd")
      await fs.mkdir(openxdDir, { recursive: true })

      const agentsDir = path.join(openxdDir, "agents")
...
test("loads commands from .openxd/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const openxdDir = path.join(dir, ".openxd")
      await fs.mkdir(openxdDir, { recursive: true })

      const commandDir = path.join(openxdDir, "command")
...
test("loads commands from .openxd/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const openxdDir = path.join(dir, ".openxd")
      await fs.mkdir(openxdDir, { recursive: true })

      const commandsDir = path.join(openxdDir, "commands")
...
test("does not try to install dependencies in read-only OPENXD_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.OPENXD_CONFIG_DIR
  process.env.OPENXD_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.OPENXD_CONFIG_DIR
    else process.env.OPENXD_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable OPENXD_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
  ...
  const run = spyOn(BunProc, "run").mockImplementation(async (_cmd, opts) => {
    const mod = path.join(opts?.cwd ?? "", "node_modules", "@openxd-ai", "plugin")
    await fs.mkdir(mod, { recursive: true })
    await Filesystem.write(
      path.join(mod, "package.json"),
      JSON.stringify({ name: "@openxd-ai/plugin", version: "1.0.0" }),
    )
    return {
      code: 0,
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "openxd.json"),
        JSON.stringify({ $schema: "https://openxd.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .openxd config
      const projectDir = path.join(dir, "project")
      const openxdDir = path.join(projectDir, ".openxd")
      await fs.mkdir(openxdDir, { recursive: true })
      ...
      // Local .openxd config with different plugins
      await Filesystem.write(
        path.join(openxdDir, "openxd.json"),
        JSON.stringify({
          $schema: "https://openxd.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .openxd config with some overlapping plugins
      await Filesystem.write(
        path.join(openxdDir, "openxd.json"),
        JSON.stringify({
          $schema: "https://openxd.ai/config.json",
...
// Note: preload.ts sets OPENXD_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .openxd config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .openxd directory config enables it
      const openxdDir = path.join(dir, ".openxd")
      await fs.mkdir(openxdDir, { recursive: true })
      await Filesystem.write(
        path.join(openxdDir, "openxd.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  globalThis.fetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/openxd")) {
      fetchedUrl = urlStr
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(fetchedUrl).toBe("https://example.com/.well-known/openxd")
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "openxd.json")
    expect(await Config.resolvePluginSpec("oh-my-openxd@2.4.3", file)).toBe("oh-my-openxd@2.4.3")
...
describe("deduplicatePlugins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-openxd@2.4.3", "file:///project/.openxd/plugin/oh-my-openxd.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.openxd/plugin/demo.ts", "file:///project/.openxd/plugin/demo.ts"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result).toEqual(["file:///project/.openxd/plugin/demo.ts"])
  })
  ...
  test("loads auto-discovered local plugins as file urls", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const openxdDir = path.join(projectDir, ".openxd")
        const pluginDir = path.join(openxdDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("OPENXD_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["OPENXD_DISABLE_PROJECT_CONFIG"]
    process.env["OPENXD_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["OPENXD_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["OPENXD_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .openxd/ directories when flag is set", async () => {
    const originalEnv = process.env["OPENXD_DISABLE_PROJECT_CONFIG"]
    process.env["OPENXD_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .openxd directory with a command
          const openxdDir = path.join(dir, ".openxd", "command")
          await fs.mkdir(openxdDir, { recursive: true })
          await Filesystem.write(path.join(openxdDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .openxd should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["OPENXD_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["OPENXD_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["OPENXD_CONFIG_DIR"]
      process.env["OPENXD_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["OPENXD_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["OPENXD_CONFIG_DIR"]
      } else {
        process.env["OPENXD_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("OPENXD_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["OPENXD_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["OPENXD_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["OPENXD_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["OPENXD_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from OPENXD_CONFIG_DIR, not project
...
describe("OPENXD_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in OPENXD_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["OPENXD_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["OPENXD_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://openxd.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["OPENXD_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["OPENXD_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in OPENXD_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["OPENXD_CONFIG_CONTENT"]
