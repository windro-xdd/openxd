...
import { CompactionV3 } from "./compaction-v3"
...
export namespace SessionProcessor {
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@openxd/SessionProcessor") {}
  ...
  > = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const compaction = yield* CompactionV3.Service
        ...
        const handleEvent = Effect.fn("SessionProcessor.handleEvent")(function* (value: StreamEvent) {
          switch (value.type) {
            ...
            case "tool-result": {
              ...
              // Apply microcompaction to large tool outputs
              const toolOutput = value.output.output
              const compacted = yield* compaction.processToolOutput({
                output: toolOutput,
                toolName: match.tool,
                partID: match.id,
              })
            ...
            case "finish-step": {
              ...
              if (ctx.snapshot) {
               ...
               // Check both overflow and headroom for compaction decision
               if (
               ...
               // Also check headroom-based compaction
               const headroomCheck = yield* compaction.checkHeadroom({
                 tokens: ctx.assistantMessage.tokens,
                 model: ctx.model,
               })
               if (headroomCheck.shouldCompact) {
                 ctx.needsCompaction = true
                 log.info("compaction.headroom_triggered", {
                   reason: headroomCheck.reason,
                   urgency: headroomCheck.urgency,
                 })
               }
              return
            }
  ...
  export const defaultLayer = Layer.unwrap(
    Effect.sync(() =>
      layer.pipe(
        Layer.provide(CompactionV3.layer),
