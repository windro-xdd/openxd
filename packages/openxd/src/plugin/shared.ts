...
export const DEPRECATED_PLUGIN_PACKAGES = ["openxd-openai-codex-auth", "openxd-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, openxdVersion: string) {
  if (!semver.valid(openxdVersion) || semver.major(openxdVersion) === 0) return
  const pkg = await readPluginPackage(target).catch(() => undefined)
  if (!pkg) return
  const engines = pkg.json.engines
  if (!isRecord(engines)) return
  const range = engines.openxd
  if (typeof range !== "string") return
  if (!semver.satisfies(openxdVersion, range)) {
    throw new Error(`Plugin requires openxd ${range} but running ${openxdVersion}`)
  }
}
