...
export namespace Plugin {
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@openxd/Plugin") {}
      ...
      const cache = yield* InstanceState.make<State>(
        Effect.fn("Plugin.state")(function* (ctx) {
          ...
          const client = createOpencodeClient({
            ...
            headers: Flag.OPENXD_SERVER_PASSWORD
              ? {
                  Authorization: `Basic ${Buffer.from(`${Flag.OPENXD_SERVER_USERNAME ?? "openxd"}:${Flag.OPENXD_SERVER_PASSWORD}`).toString("base64")}`,
                }
              : undefined,
          ...
          const plugins = Flag.OPENXD_PURE ? [] : (cfg.plugin ?? [])
          if (Flag.OPENXD_PURE && cfg.plugin?.length) {
