...
function createThemeInstaller(meta: TuiConfig.PluginMeta, root: string, spec: string): TuiTheme["install"] {
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".openxd"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".openxd", "themes")
...
function defaultPluginMeta(state: RuntimeState): TuiConfig.PluginMeta {
  return {
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".openxd", "tui.json"),
  }
}
  ...
  async function load(api: Api) {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const plugins = Flag.OPENXD_PURE ? [] : (config.plugin ?? [])
        if (Flag.OPENXD_PURE && config.plugin?.length) {
