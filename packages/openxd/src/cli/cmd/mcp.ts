    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.outro("Add servers with: openxd mcp add")
          return
        }
...
async function resolveConfigPath(baseDir: string, global = false) {
  ...
  if (!global) {
    candidates.push(path.join(baseDir, ".openxd", "openxd.json"), path.join(baseDir, ".openxd", "openxd.jsonc"))
  }
      ...
      async fn() {
        ...
        if (type === "local") {
          const command = await prompts.text({
            ...
            placeholder: "e.g., openxd x @modelcontextprotocol/server-filesystem",
            validate: (x) => (x && x.length > 0 ? undefined : "Required"),
          })
          ...
          const response = await fetch(serverConfig.url, {
            ...
            body: JSON.stringify({
              ...
              params: {
                ...
                capabilities: {},
                clientInfo: { name: "openxd-debug", version: Installation.VERSION },
              },
              id: 1,
            }),
          })
