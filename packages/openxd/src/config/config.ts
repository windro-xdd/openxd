...
export namespace Config {
  ...
  function systemManagedConfigDir(): string {
    switch (process.platform) {
      case "darwin":
        return "/Library/Application Support/openxd"
      case "win32":
  ...
  export function managedConfigDir() {
    return process.env.OPENXD_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
  }
  ...
  export async function installDependencies(dir: string, input?: InstallInput) {
    ...
    await BunProc.run(
      ...
      if (err instanceof Process.RunFailedError) {
        ...
        if (Flag.OPENXD_STRICT_CONFIG_DEPS) {
  ...
  export async function needsInstall(dir: string) {
    ...
    const mod = path.join(dir, "node_modules", "@openxd-ai", "plugin")
  ...
  export const Server = z
    .object({
      ...
      mdnsDomain: z.string().optional().describe("Custom domain name for mDNS service (default: openxd.local)"),
  ...
  export const Info = z
    .object({
      ...
      server: Server.optional().describe("Server configuration for openxd serve and web commands"),
      command: z
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@openxd/Config") {}

  function globalConfigFile() {
    const candidates = ["openxd.jsonc", "openxd.json", "config.json"].map((file) =>
      ...
      Effect.gen(function* () {
        ...
        const loadConfig = Effect.fnUntraced(function* (
          ...
          const normalized = (() => {
            ...
            delete copy.tui
            log.warn("tui keys in openxd config are deprecated; move them to tui.json", { path: source })
            return copy
          })()
        ...
        const loadGlobal = Effect.fnUntraced(function* () {
          let result: Info = pipe(
            ...
            mergeDeep(yield* loadFile(path.join(Global.Path.config, "openxd.json"))),
            mergeDeep(yield* loadFile(path.join(Global.Path.config, "openxd.jsonc"))),
          )
        ...
        const loadInstanceState = Effect.fnUntraced(function* (ctx: InstanceContext) {
          ...
          for (const [key, value] of Object.entries(auth)) {
            if (value.type === "wellknown") {
              ...
              log.debug("fetching remote config", { url: `${url}/.well-known/openxd` })
              const response = yield* Effect.promise(() => fetch(`${url}/.well-known/openxd`))
              if (!response.ok) {
              ...
              result = mergeConfigConcatArrays(
                ...
                yield* loadConfig(JSON.stringify(remoteConfig), {
                  dir: path.dirname(`${url}/.well-known/openxd`),
                  source: `${url}/.well-known/openxd`,
                }),
              )
          ...
          if (Flag.OPENXD_CONFIG) {
            result = mergeConfigConcatArrays(result, yield* loadFile(Flag.OPENXD_CONFIG))
            log.debug("loaded custom config", { path: Flag.OPENXD_CONFIG })
          }

          if (!Flag.OPENXD_DISABLE_PROJECT_CONFIG) {
            for (const file of yield* Effect.promise(() =>
              ConfigPaths.projectFiles("openxd", ctx.directory, ctx.worktree),
            )) {
          ...
          if (Flag.OPENXD_CONFIG_DIR) {
            log.debug("loading config from OPENXD_CONFIG_DIR", { path: Flag.OPENXD_CONFIG_DIR })
          }

          const deps: Promise<void>[] = []

          for (const dir of unique(directories)) {
            if (dir.endsWith(".openxd") || dir === Flag.OPENXD_CONFIG_DIR) {
          ...
          if (process.env.OPENXD_CONFIG_CONTENT) {
            result = mergeConfigConcatArrays(
              result,
              yield* loadConfig(process.env.OPENXD_CONFIG_CONTENT, {
                dir: ctx.directory,
                source: "OPENXD_CONFIG_CONTENT",
              }),
            )
            log.debug("loaded custom config from OPENXD_CONFIG_CONTENT")
          }
          ...
          if (active?.active_org_id) {
            yield* Effect.gen(function* () {
              ...
              if (token) {
                process.env["OPENXD_CONSOLE_TOKEN"] = token
                Env.set("OPENXD_CONSOLE_TOKEN", token)
              }
          ...
          if (Flag.OPENXD_PERMISSION) {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.OPENXD_PERMISSION))
          }
          ...
          if (Flag.OPENXD_DISABLE_AUTOCOMPACT) {
            result.compaction = { ...result.compaction, auto: false }
          }
          if (Flag.OPENXD_DISABLE_PRUNE) {
