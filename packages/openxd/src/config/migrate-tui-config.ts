...
export async function migrateTuiConfig(input: MigrateInput) {
  const openxd = await openxdFiles(input)
  for (const file of openxd) {
...
async function openxdFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.OPENXD_DISABLE_PROJECT_CONFIG
    ? []
  ...
  if (Flag.OPENXD_CONFIG) files.push(Flag.OPENXD_CONFIG)
