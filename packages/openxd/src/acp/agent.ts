...
export namespace ACP {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `openxd auth login` in the terminal",
        name: "Login with openxd",
        id: "openxd-login",
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // OpenXD stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
    ...
    const openxdProvider = providers.find((p) => p.id === "openxd")
    if (openxdProvider) {
      if (openxdProvider.models["big-pickle"]) {
        return { providerID: ProviderID.openxd, modelID: ModelID.make("big-pickle") }
      }
      const [best] = Provider.sort(Object.values(openxdProvider.models))
      if (best) {
        return {
