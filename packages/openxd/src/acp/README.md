...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for openxd.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal openxd sessions
...
openxd acp --cwd /path/to/project
```
...
```bash
OPENXD_ENABLE_QUESTION_TOOL=1 openxd acp
```
...
- **Terminal Integration**: Full terminal support via openxd's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | openxd acp
```
...
ACP sessions map cleanly to openxd's internal session model:
