...
Replace runtime Bun shell template-tag usage in `packages/openxd/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/openxd/src` is removed except:
