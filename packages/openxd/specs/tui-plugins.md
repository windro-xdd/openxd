...
- Author package entrypoint is `@openxd-ai/plugin/tui`.
...
```json
{
  "$schema": "https://openxd.ai/tui.json",
  "theme": "smoke-theme",
  "plugin": ["@acme/openxd-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  }
}
...
- Import types from `@openxd-ai/plugin/tui`.
- `@openxd-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@openxd-ai/plugin/tui"
...
- The value is a semver range checked against the running OpenXD version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.openxd` is absent, no check is performed (backward compatible).
...
- `openxd plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `openxd plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.openxd` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.openxd`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.openxd`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.openxd` dir or `OPENXD_CONFIG_DIR`, OpenXD installs `@openxd-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@openxd-ai/plugin/tui`.
...
- Local plugins persist installed themes under the local `.openxd/themes` area near the plugin config source.
...
- `--pure` / `OPENXD_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.openxd/plugins/tui-smoke.tsx`
- Local smoke config: `.openxd/tui.json`
- Local smoke theme: `.openxd/plugins/smoke-theme.json`
